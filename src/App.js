...
export default function App(){
    //individual components as well as GUN instance and control
    const gun = GUN();

    //current SVG displayed
    const [svg, setSVG] = useState({
        //states ensure the page is reloaded/updated when the values changes
        title: "Ethereum Logo",
    ...
    //callback func to update the SVG from the selector
    const changeCurrentSVG = (selectionTitle) => {
        //ensure the prev selected SVG is pushed to GUN
        gun.get(svg.title).put({
            title: svg.title,
            svg: svg.svg,
        });

        //get new SVG and update the state
